...
You can get a list of changes made to a document using the [`_changes` endpoint](../api/database.html#get-changes).
However,
...
Cloudant using the [checkpoint](replication_guide.html#checkpoints) mechanism
...
There are advanced replication API options [available](../api/advanced_replication.html) which enable you to increase or decrease the amount of computing power used during replication.
For example:
