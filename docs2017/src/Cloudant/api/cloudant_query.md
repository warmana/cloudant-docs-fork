...
Miscellaneous | `$mod`    | [Divisor, Remainder] | Divisor and Remainder are both positive or negative integers. Non-integer values result in a [404 status](/docs/Cloudant/api/http.html#404). Matches documents where (`field % Divisor == Remainder`) is true, and only when the document field is an integer.
...
A query where the Divisor or Remainder is a non-integer returns a [404 status](/docs/Cloudant/api/http.html#404).
