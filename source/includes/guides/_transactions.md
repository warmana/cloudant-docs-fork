...
To create a unique identifier to use in your application, such as an `order_id`, call the `GET _uuids` endpoint on the Cloudant API and the database will generate an identifier for you. See [Get/_UUIDS](advanced.html#get-/_uuids)  in the Advanced section of the Cloudant API reference for more information. The same endpoint can be used to generate multiple ids by adding a `count` parameter, for example, `/_uuids?count=10`.
