...
external | The billable data size that is also known as the uncompressed user data volume. It must match the `data_size` field.
active | The actual amount of data that is close to the `file` field when the view is freshly compacted.
